...
`WasteContainer` tracks the physical fleet. Key fields: `base_early_empty_cost_eur` (static base cost, renamed from `early_empty_cost_eur`), `overflow_penalty_eur` (capped penalty for overflowing), `fill_ratio`. Use `compute_dynamic_early_empty_cost(container)` to get the fill-adjusted cost. Standalone emptying: `POST /api/v1/containers/{id}/early-empty`.
...
- **Proactive container management**: The worker applies expected-value logic each cycle (`early_cost < penalty * fill_ratio`) and routes proactive early-empties through Decision Center before acting. APPROVE → trigger; REJECT → skip; anything else → hold.
